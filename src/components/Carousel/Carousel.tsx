    ...
    <div className="carousel">
      <div className="carousel__inner">
        {pictures.map((picture, index) => (
          <img
            className={index === currentPicture ? 'active' : ''}
            key={picture.url}
            src={picture.url}
            alt={`Slide ${index}`}
          />
      ...
      <button className="carousel__left" aria-label="Précédent" onClick={previous}>
      ...
      <button className="carousel__right" aria-label="Suivant" onClick={next}>
        <svg viewBox="0 0 48 80" fill="#ffffff" xmlns="http://www.w3.org/2000/svg">
          <path
